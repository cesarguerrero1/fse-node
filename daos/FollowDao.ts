...
class FollowDao implements FollowDaoI {
    ...
    */
    async findPeopleIFollow(uid: string): Promise<Follow[]> {
        return await FollowModel.find({userFollowing: uid}).populate("userFollowing", {_id:1}).populate("userFollowed", {_id:1}).exec();
    }
    ...
    */
    async findPeopleWhoFollowMe(uid: string): Promise<Follow[]> {
        return await FollowModel.find({userFollowed: uid}).populate("userFollowing", {_id:1}).populate("userFollowed", {_id:1}).exec();
    }
